...
public class ProdutoRest extends UtilRest {
	...
	public Response inserir(String produtoParam) {
		try {
			...
			JDBCProdutoDAO jdbcProduto = new JDBCProdutoDAO(conexao);
			String msg = "";	
			boolean consultaMarca = jdbcProduto.consultaMarca(produto.getMarcaId());
			if(consultaMarca) {			
				boolean consulta = jdbcProduto.consultaDuplicidade(produto);
				if(consulta) {
					...
					msg = "Já existe um produto com esta marca, modelo e capacidade.";
				};				
			} else {
				msg = "Marca não existente. Atualize a página para ver os registros atuais.";				
			};
			conec.fecharConexao();
			return this.buildResponse(msg);
		} catch(Exception e) {
